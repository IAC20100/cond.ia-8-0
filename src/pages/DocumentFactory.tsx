...
export default function DocumentFactory() {
  ...
  const handleClientSelect = (clientId: string) => {
    ...
    setFieldValues(prev => {
      ...
      // Update edited content with filled values
      const filled = fillingTemplate.content.replace(/\[(.*?)\]/g, (match: string, p1: string) => {
        return newValues[p1] || `[${p1}]`;
      });
                ...
                >
                  <div className="flex justify-between items-start mb-6">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-3 py-1 bg-slate-100 dark:bg-zinc-800 text-[0.625rem] font-black uppercase tracking-widest rounded-lg">
                        {template.category}
                      </span>
                      <button 
                  ...
                  <div className="bg-slate-50 dark:bg-zinc-800/50 rounded-2xl p-4 mb-6 border border-slate-100 dark:border-zinc-800">
                    <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 mb-1">
                      ...
                      <span className="text-[0.625rem] font-black uppercase tracking-wider">Base Jurídica</span>
                    </div>
              ...
              <div className="grid grid-cols-1 gap-4">
                {extractFields(fillingTemplate?.content || '').map(field => (
                  <div key={field} className="space-y-1.5">
                    <label className="text-[0.625rem] font-black uppercase tracking-widest text-slate-400 ml-2">
                      {field}
                    </label>
                    <input 
                      type="text"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-2">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-1 rounded-lg text-[0.625rem] font-black uppercase tracking-widest transition-all ${
                  isEditingPreview 
                    ? 'bg-emerald-500 text-white' 
                    : 'bg-slate-100 dark:bg-zinc-800 text-slate-500 hover:text-blue-600'
                }`}
              >
                {isEditingPreview ? (
                  <><CheckCircle2 className="w-3 h-3" /> Concluir Edição</>
                ) : (
                  <><FileEdit className="w-3 h-3" /> Editar Texto Final</>
                )}
              </button>
            </div>
            <div className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-inner h-[600px] overflow-y-auto font-serif text-black leading-relaxed whitespace-pre-wrap text-sm">
              {isEditingPreview ? (
                <textarea
                  value={editedContent}
                  onChange={(e) => setEditedContent(e.target.value)}
                  className="w-full h-full p-4 bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-serif text-sm leading-relaxed resize-none text-black"
                  placeholder="Edite o conteúdo final do documento aqui..."
                />
              ) : (
                <div ref={printRef} className="bg-white text-zinc-900 min-h-full relative p-16 pdf-content font-sans" style={{ color: '#18181b' }}>
                  ...
                  <div className="relative z-10 mb-16 flex justify-between items-start">
                    <div className="flex flex-col gap-6">
                      {companyLogo ? (
                        <div className="bg-white rounded-2xl flex items-center justify-center border border-zinc-900 p-4" style={{ width: '128px', height: '128px' }}>
                          <img 
                            src={companyLogo} 
                            alt="Logo" 
                        ...
                        <div className="bg-zinc-900 rounded-2xl flex items-center justify-center text-white" style={{ width: '96px', height: '96px' }}>
                      ...
                      <div>
                        <h1 className="text-4xl font-black text-zinc-900 tracking-tight leading-none mb-3 uppercase">
                          {fillingTemplate?.title}
                        </h1>
                        <div className="flex items-center gap-3">
                          <span className="h-1 w-12 bg-zinc-900 rounded-full"></span>
                          <p className="text-xs font-black text-zinc-900 uppercase tracking-[0.2em]">
                            {companyData?.name || 'IA COMPANY'}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="text-right flex flex-col items-end gap-4 pt-4">
                      <div className="space-y-1">
                        <p className="text-[0.625rem] font-black text-zinc-400 uppercase tracking-widest">Identificação do Documento</p>
                        <p className="text-sm font-bold text-zinc-900 tabular-nums">#{Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[0.625rem] font-black text-zinc-400 uppercase tracking-widest">Data de Emissão</p>
                        <p className="text-sm font-bold text-zinc-900">{format(new Date(), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}</p>
                      </div>
                    </div>
                  </div>

                  {/* Info Bar */}
                  <div className="relative z-10 grid grid-cols-3 gap-8 p-6 bg-white rounded-2xl border border-zinc-900 mb-16">
                    <div>
                      <p className="text-[0.5625rem] font-black text-zinc-900 uppercase tracking-widest mb-1">Endereço</p>
                      <p className="text-[0.6875rem] text-zinc-900 font-medium leading-relaxed">
                        {companyData?.address || 'Endereço não informado'}
                      </p>
                    </div>
                    <div>
                      <p className="text-[0.5625rem] font-black text-zinc-900 uppercase tracking-widest mb-1">Contato</p>
                      <p className="text-[0.6875rem] text-zinc-900 font-medium">
                        {companyData?.phone || 'Telefone não informado'}
                      </p>
                      <p className="text-[0.6875rem] text-zinc-900 font-medium">
                        {companyData?.email || 'E-mail não informado'}
                      </p>
                    </div>
                    <div>
                      <p className="text-[0.5625rem] font-black text-zinc-900 uppercase tracking-widest mb-1">Registro</p>
                      <p className="text-[0.6875rem] text-zinc-900 font-medium">
                        {companyData?.document ? `CNPJ: ${companyData.document}` : 'CNPJ não informado'}
                      </p>
                      <p className="text-[0.6875rem] text-zinc-900 font-medium">
                        {companyData?.website || ''}
                      </p>
                  ...
                  {/* Content Area */}
                  <div className="relative z-10 text-justify text-[0.9375rem] leading-[1.8] text-zinc-700 space-y-6 mb-24 px-2">
                  ...
                  <div className="relative z-10 mt-auto pt-12 border-t border-zinc-900 page-break-inside-avoid">
                    <div className="grid grid-cols-2 gap-20 w-full mb-16">
                      <div className="flex flex-col items-start">
                        <div className="w-full h-px bg-zinc-900 mb-4"></div>
                        <p className="text-[0.625rem] font-black text-zinc-900 uppercase tracking-widest mb-1">Responsável / Síndico</p>
                        <p className="text-xs font-bold text-zinc-900">{companyData?.name || 'Assinatura Autorizada'}</p>
                      </div>
                      <div className="flex flex-col items-start">
                        <div className="w-full h-px bg-zinc-900 mb-4"></div>
                        <p className="text-[0.625rem] font-black text-zinc-900 uppercase tracking-widest mb-1">Testemunha / Condômino</p>
                        <p className="text-xs font-bold text-zinc-900">Ciente em: ____/____/____</p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className="text-[0.625rem] font-black text-emerald-600 uppercase tracking-widest">Documento Assinado</span>
                          <span className="text-[0.5625rem] text-zinc-400 font-medium">Validado digitalmente via Condfy.IA</span>
                        </div>
                      </div>
                      
                      <div className="max-w-[300px] text-right">
                        <p className="text-[0.5625rem] text-zinc-400 font-medium leading-relaxed italic">
                          Este documento possui validade jurídica conforme MP nº 2.200-2/2001, que institui a Infraestrutura de Chaves Públicas Brasileira - ICP-Brasil.
                        </p>
                      </div>
                    </div>
                  </div>
