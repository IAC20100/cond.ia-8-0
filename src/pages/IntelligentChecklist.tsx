  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute flex flex-col items-center justify-center text-center">
        <span className="text-[0.625rem] uppercase tracking-widest text-white/60 mb-1 w-24">Meta de Manutenção Preventiva</span>
          ...
          <div className="flex items-center gap-6 text-white/60 overflow-x-auto pb-2 md:pb-0 w-full md:w-auto custom-scrollbar">
            {[
              ...
              >
                ...
                <span className="text-[0.625rem] uppercase tracking-wider">{item.label}</span>
              </button>
            ))}
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-y-auto pr-2 custom-scrollbar" style={{ maxHeight: 'calc(100vh - 280px)' }}>
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col shadow-lg backdrop-blur-md flex-1">
              ...
              <div className="mt-8">
                <h3 className="text-[0.625rem] font-bold text-white uppercase tracking-widest mb-4">Manutenção por Torre (% Concluída)</h3>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col shadow-lg backdrop-blur-md relative">
              ...
              <div className="flex justify-between items-start">
                ...
                <div className="flex flex-col items-end text-right">
                  <span className="text-[0.625rem] font-bold text-white/60 uppercase">Tarefas Críticas:</span>
                ...
                {upcomingTasks.length > 0 ? upcomingTasks.map((task, i) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-[0.625rem] text-white/50">{format(parseISO(task.nextDate), 'dd/MM')}</p>
                    </div>
                  </div>
                )) : (
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-lg backdrop-blur-md flex-1">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr>
                      <th className="pb-2 text-[0.625rem] font-bold text-white/50 uppercase w-6 text-center">#</th>
                      {weekDays.map((day, i) => (
                        <th key={i} className="pb-2 text-[0.625rem] font-bold text-white/80 uppercase text-center">
                    ...
                    {[1, 2, 3, 4, 5].map((rowNum, rowIndex) => {
                      return (
                        <tr key={rowIndex} className="border-t border-white/5">
                          <td className="py-2 font-bold text-white/40 text-center text-[0.625rem]">{rowNum}</td>
                          {weekDays.map((colDay, colIndex) => {
                            ...
                            return (
                              <td key={colIndex} className="py-1 px-1">
                                {task ? (
                                  <div className="bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-[0.5625rem] px-1 py-1 rounded text-center truncate w-14 mx-auto" title={task.item}>
                                    {task.item}
                                  </div>
                                ) : (
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-lg backdrop-blur-md">
              ...
              <h3 className="text-[0.625rem] font-bold text-white/50 uppercase mb-3">Últimos Avisos Oficiais</h3>
        ...
        >
          ...
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="w-12 h-12 bg-blue-600 rounded-lg flex items-center justify-center overflow-hidden">
              {companyLogo ? (
                <img src={companyLogo} alt="Logo" className="w-full h-full object-contain p-1" referrerPolicy="no-referrer" />
              ) : (
          ...
          {/* Info Box */}
          <div className="bg-white border border-zinc-900 rounded-2xl p-8 mb-10 grid grid-cols-2 gap-x-12 gap-y-6 break-inside-avoid">
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-zinc-900 mb-1">Condomínio/Prédio:</p>
              <p className="text-xl font-bold border-b border-zinc-900 pb-1">{selectedClient?.name || '________________________________'}</p>
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-zinc-900 mb-1">Data da Inspeção:</p>
              <p className="text-xl font-bold border-b border-zinc-900 pb-1">____ / ____ / 20____</p>
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-zinc-900 mb-1">Responsável Técnico:</p>
              <p className="text-xl font-bold border-b border-zinc-900 pb-1">________________________________</p>
            </div>
            <div>
              <p className="text-xs font-black uppercase tracking-widest text-zinc-900 mb-1">Assinatura:</p>
              <div className="h-20 flex items-end justify-center border-b border-zinc-900 pb-1 relative">
                {companySignature && (
                  <img 
                    src={companySignature} 
                    alt="Assinatura" 
                    className="max-h-full max-w-full object-contain mb-[-12px] relative z-10" 
          ...
          {/* Table */}
          <div className="border border-zinc-900 rounded-2xl overflow-hidden mb-10">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-zinc-50 border-b border-zinc-900">
                  <th className="py-3 px-2 text-center text-[0.625rem] font-black uppercase tracking-widest text-zinc-900 w-12">OK</th>
                  <th className="py-3 px-2 text-center text-[0.625rem] font-black uppercase tracking-widest text-zinc-900 w-12">NOK</th>
                  <th className="py-3 px-2 text-center text-[0.625rem] font-black uppercase tracking-widest text-zinc-900 w-12">N/A</th>
                  <th className="py-3 px-4 text-left text-[0.625rem] font-black uppercase tracking-widest text-zinc-900">Item / Descrição</th>
                  <th className="py-3 px-4 text-left text-[0.625rem] font-black uppercase tracking-widest text-zinc-900 w-48">Observações</th>
                </tr>
              </thead>
                ...
                {clientSchedules.map((schedule, idx) => (
                  <tr key={idx} className="break-inside-avoid">
                    <td className="py-4 px-2 text-center">
                      <div className="w-5 h-5 border-2 border-zinc-900 rounded mx-auto" />
                    </td>
                    <td className="py-4 px-2 text-center">
                      <div className="w-5 h-5 border-2 border-zinc-900 rounded mx-auto" />
                    </td>
                    ...
                    <td className="py-4 px-4">
                      ...
                      <p className="text-[0.625rem] text-zinc-500 uppercase tracking-tight">
          ...
          <div className="mt-auto pt-20 text-center break-inside-avoid">
            <p className="text-[0.625rem] text-zinc-900 leading-relaxed font-bold">
